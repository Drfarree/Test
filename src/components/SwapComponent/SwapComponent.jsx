...
function SwapComponent(props) {
  ...
  async function changeAmount(e) {
    ...
    const ethByToken = prices.ratio / 10 ** decimals;
    //TODO: Cambiar
    if (e.target.value && prices)
      if (tokenOne.address === tokenList[0].address)
        setTokenTo((e.target.value / ethByToken).toFixed(3));
      else setTokenTo((e.target.value * ethByToken).toFixed(5));
  }
  ...
  const fetchDexSwap = async () => {
    ...
    if (tokenOne.address === tokenList[0].address) fnToCall = SwapETHforTokens;
    else fnToCall = approveAndSwapTokens;

    fnToCall(tokenFromAmount)
      .then(() => fnCallbackSucess())
      .catch(() => fnCallbackError());
  };
    ...
    <>
      ...
      >
        ...
        <div className="flex flex-col justify-start items-start pl-[30px] pr-[30px]">
          <h1 className="m-4 text-2xl font-extrabold leading-none tracking-tight text-gray-900 dark:text-white uppercase">
            <span className="text-[35px] font-[650]">S</span>wap
          </h1>
          <div className="relative">
            ...
            <div
              className="bg-[#3a4157] w-[35px] h-[35px] items-center justify-center flex rounded-[8px] absolute top-[80px] left-[45%] text-[#5F6783] border-[3px] border-solid  border-[#0E111B] text-[18px] duration-[0.3s] hover:text-white hover:cursor-pointer"
              onClick={switchTokens}
            >
              <ArrowDownOutlined className=" switchArrow" />
            </div>
            <div
              className="swapAsset top-[36px] right-[20px]"
              onClick={() => openModal(1)}
            >
              <img
                src={tokenOne.img}
            ...
            <div
              className="swapAsset top-[135px] right-[20px]"
              onClick={() => openModal(2)}
            >
              <img
                src={tokenTwo.img}
